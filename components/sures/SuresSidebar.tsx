...
function SidebarInnerContent({
  ...
  const displayName = user?.name || user?.email?.split("@")[0] || "Sures Admin"
  ...
  return (
    <>
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-700">
        <Link href="/sures/dashboard" className="flex items-center gap-2">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-lg">
            <Shield className="w-5 h-5 text-white" />
          </div>
          <div>
            <span className="font-bold text-slate-900 dark:text-white">Sures Portal</span>
            <span className="block text-xs text-slate-500 dark:text-slate-400">Carrier Dashboard</span>
          </div>
        </Link>
      </div>

      {/* User info */}
      <div className="p-4 mx-4 mt-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl">
        ...
        <div className="mt-3 pt-3 border-t border-emerald-200 dark:border-emerald-700">
          <span className="inline-flex items-center px-2 py-1 bg-emerald-100 dark:bg-emerald-800 text-emerald-700 dark:text-emerald-200 text-xs font-medium rounded-full">
            {devMode ? "Dev Mode" : "Sures Administrator"}
          </span>
        </div>
      </div>
